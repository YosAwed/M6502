...
int cmd_print(basic_state_t* state, parser_state_t* parser) {
    ...
    while (1) {
        ...
        if (!first) trailing_semicolon = false;
        if (token.type == TOKEN_STRING) {
            printf("%s", token.value.string);
            free(token.value.string);
        } else if (token.type == TOKEN_NUMBER) {
            printf("%g", numeric_to_double(token.value.number));
        } else if (token.type == TOKEN_VARIABLE) {
            variable_t* var = find_variable(state, token.value.string);
            if (!var) { set_error(state, ERR_UNDEF_STATEMENT, "Variable not found"); free(token.value.string); return -1; }
            if (var->type == VAR_STRING) printf("%s", var->value.str.data ? var->value.str.data : "");
            else printf("%g", numeric_to_double(var->value.num));
            free(token.value.string);
        } else if (token.type == TOKEN_DELIMITER && token.value.operator == ',') {
            printf("\t");
            first = false; continue;
        } else if (token.type == TOKEN_DELIMITER && token.value.operator == ';') {
            trailing_semicolon = true; first = false; continue;
        } else if (token.type == TOKEN_KEYWORD && token.value.keyword_id == 0x9D) { // TAB
            ...
            token_t n = get_next_token(state, parser);
            if (n.type != TOKEN_NUMBER) { set_error(state, ERR_TYPE_MISMATCH, "Numeric expected in TAB"); return -1; }
            int count = (int)numeric_to_double(n.value.number);
            ...
            set_error(state, ERR_SYNTAX, "Invalid PRINT argument");
            return -1;
        }
        first = false;
        // lookahead for next separator; if it's ',' or ';' we handle in the next loop
