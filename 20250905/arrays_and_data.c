...
int cmd_dim(basic_state_t* state, parser_state_t* parser_ptr) {
    ...
    while (true) {
        ...
        // 既存変数チェック
        variable_t* existing_var = find_variable(state, var_token.value.string);
        if (existing_var && (existing_var->type == VAR_ARRAY_NUMERIC || 
                            existing_var->type == VAR_ARRAY_STRING)) {
            set_error(state, ERR_REDIMENSIONED_ARRAY, NULL);
